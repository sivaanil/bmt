...
class Curl {
	...
	public function create($url)
	{
		...
		if($url = $this->_ci->config->item('server').'login/userlogin'){
			$this->methodislogin = true;
		}
	...
	public function execute()
	{
		...
		if($this->methodislogin){
			...
			$resobject = explode("\n\r", $this->response);
			$headerobject = explode("\n",$resobject[0]);

			$headerarray = $this->getHeaderDetails($headerobject);

			if(in_array('Authtoken', array_keys($headerarray))){	
				//list($headkey, $headervalue) = explode(":",$headerobject[4]); 
				$headervalues = array("user_data"=>array('Authtoken'=>trim($headerarray['Authtoken'])));
				$this->_ci->session->set_userdata($headervalues);
			}

			$this->response = $resobject[1];
		}	
	...
	private function getHeaderDetails($headerlist){
		$resultdata = array();	
		if(count($headerlist)){
			foreach($headerlist as $keyvalue){
				if(strpos($keyvalue, ":")){
					list($headkey, $headvalue) = explode(':',$keyvalue);
					$resultdata[$headkey] = $headvalue; 
				}	
			}
		}
		return $resultdata;
