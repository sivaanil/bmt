...
class REST
{
    ...
    function __construct($config = array())
    {
        ...
        if(empty($config))
        {
            $config['server'] = $this->_ci->config->item('server');
        }
    ...
    {
        $this->_ci->curl->http_header($header,$content);
    }
    ...
    protected function _call($method, $uri, $params = array(), $format = NULL)
    {
        ...
        if ($this->send_cookies != '')
        {
         ...
         /*This Comment by Ramesh on 16/12/2014 for Creation of Deal*/   
        // Set the Content-Type (contributed by https://github.com/eriklharper)
        //$this->http_header('Content-type', $this->mime_type); // uncomment this
