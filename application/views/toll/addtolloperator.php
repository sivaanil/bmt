...
//echo "<pre>"; print_r($toll_staff_details);exit;
?>
<div class="col-md-9 right_menu">
    <form class="form-horizontal" style="width:93%; margin:0 auto;" method="post" id="addsemiadmin" name="addsemiadmin" action="<?php echo base_url('tollcenter/addtolloperator'); ?>">
        ...
        <input type="hidden" name="tolltype" id="tolltype" value="<?php echo @$listTollCenterLocations->TollCenterDetails[0]->toll_type_id; ?>">
        <?php if ($listTollCenterLocations->TollCenterDetails[0]->toll_type_id == 2) { ?>
            <div class="row">
                <div class="col-md-8 col-sm-8">
                    <div class="form-group">
                        <select style="color:#0C0C0C;" class="form-control" id="toll_center_loaction" name="toll_center_loaction" >
                            <?php
                            foreach ($listTollCenterLocations->TollCenterDetails as $key => $value) {
                                ?>
                                <option value="<?php echo @$value->tc_id; ?>"><?php echo @$value->city . " - " .@$value->tc_location . " - " . @$value->tc_name; ?></option>
                            <?php }
                            ?>
    ...
    <div class="row">
        <table class="table" style="width:100%; margin-left:-10%; border: 1px #DDD solid;">
            <thead>
                <tr>
                    <th>Toll Center</th>
                    <th>Name</th>
            ...
            <tbody>
                ...
                if (isset($toll_staff_details) && !empty($toll_staff_details)) {
                    foreach (@$toll_staff_details as $key => $value) {
                        ...
                        ?>
                        <tr>
                            <td id="toll_center_loaction_<?php echo $value->ts_id ?>"><?php echo @$value->city . ' - ' . @$value->tc_name . ' - ' . $value->tc_location ?></td>
                           ...
                           <!--  <td><?php //echo  @$role_name    ?></td> -->
                            <td>
                                ...
                                <button type="button" id="delete_vehicle" class="btn btn-danger right_menu-button-small" ><a href="javascript:void(0);" onclick="delete_semiadmin('<?php echo $value->ts_id ?>')"><?php
                                        if (isset($value->status_flag) && $value->status_flag == 0)
                                            echo "Inactive";
                                        if (isset($value->status_flag) && $value->status_flag == 1)
                                            echo "Active";
                                        ?></a></button>
                            </td>
                        </tr>
