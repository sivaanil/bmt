...
<div class="col-md-9 right_menu">
  <form class="form-horizontal" style="width:93%; margin:0 auto;" method="post" id="addsemiadmin" name="addsemiadmin" action="<?php echo base_url('tollcenter/addsemiadmin');?>">
    <div class="row">
      ...
      <div class="col-md-4 col-sm-4" id="toll_center_name_div">
    ...
    <table class="table" style="width:90%; margin-left:5%; border: 1px #DDD solid;">
      ...
      if(isset($toll_staff_details) && !empty($toll_staff_details))
      {
        foreach (@$toll_staff_details as $key => $value) 
        {
         ...
         ?>
          <tr>
            <td id="tc_location_<?php echo $value->ts_id ?>"><?php echo  @$value->tc_location?></td>
            <td id="tc_name_<?php echo $value->ts_id ?>"><?php echo  @$value->tc_name?></td>
            ...
            <td id="role_<?php echo $value->ts_id ?>"><?php echo  @$role_name?></td>
            <td>
...
<script>
    function getTollCenterLocation(){
  	...
  	$("#toll_center_loaction").html('');
        console.log(tolltype_id);
  	if(tolltype_id==2){
            console.log('iff');
            $("#toll_center_loaction").attr("multiple",'');
            $("#toll_center_name_div").hide();
  	}else{
            console.log('elseeee');
            $("#toll_center_loaction").removeAttr("multiple");
            $("#toll_center_name_div").show();
        }
        if(tolltype_id != '')
        {
          $.ajax({
                "url":"<?php echo base_url('tollcenter/getTollCenters'); ?>",
                "type" : "POST",
                "dataType":"json",
...
$(document).ready(function(){
    $('#addsemiadmin').validate({
      ...
      messages:{
          tolltype:{
                    required:"Please Select Toll Type",
               },
