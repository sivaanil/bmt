...
class Tollcenter extends CI_Controller {
    ...
    public function index() {
        ...
        // echo "<pre>";print_r($toll_center_details);exit;
//		chackStatus($toll_center_details);
        ...
        $this->form_validation->set_rules('nh_type1', 'Type', 'required');
        $this->form_validation->set_rules('nh_type2', 'Type', 'required');
        ...
        if (isset($_POST['toll_type']) && $_POST['toll_type'] == "orr") {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $this->requestobj['toll_type'] = $this->input->post('toll_type');
            $this->requestobj['lngone'] = $lng;
            $this->requestobj['latone'] = $lat;
            $this->requestobj['lngtwo'] = $lngsec;
            $this->requestobj['lattwo'] = $latsec;
            ...
            $this->requestobj['nh_type1'] = $this->input->post('nh_type1');
            $this->requestobj['nh_type2'] = $this->input->post('nh_type2');
            if ($_POST['toll_type'] == "orr") {
                $this->requestobj['orr_from_way_location'] = $this->input->post('orr_entry_from');
                $this->requestobj['orr_from_way_no_of_lanes'] = $this->input->post('orr_number_landes_from');
                $this->requestobj['orr_from_way_no_of_bmt_lanes'] = $this->input->post('orr_number_landes_from_bmt');
                $this->requestobj['orr_to_way_location'] = $this->input->post('orr1_entry_from2');
                $this->requestobj['orr_to_way_no_of_lanes'] = $this->input->post('orr1_number_landes_from2');
                $this->requestobj['orr_to_way_no_of_bmt_lanes'] = $this->input->post('orr1_number_landes_from_bmt2');
                $this->requestobj['orr_type1'] = $this->input->post('orr_type1');
                $this->requestobj['orr_type2'] = $this->input->post('orr_type2');
                $this->requestobj['city_id'] = $this->input->post('city');
            }

            if (!empty($_POST['orr_type1']) && !empty($_POST['orr_type2'])) {
                $this->requestobj['orr_type1'] = $this->input->post('orr_type1');
                $this->requestobj['orr_type2'] = $this->input->post('orr_type2');
            }
            if (isset($_POST['from_uuid']) && !empty($_POST['from_uuid'])) {
            ...
            if (isset($_POST['orr_from_uuid']) && !empty($_POST['orr_from_uuid'])) {
                $this->requestobj['orr_from_uuid'] = $this->input->post('orr_from_uuid');
                $this->requestobj['orr_from_major_id'] = $this->input->post('orr_from_major_id');
                $this->requestobj['orr_from_minor_id'] = $this->input->post('orr_from_minor_id');
            }
            if (isset($_POST['orr1_to_uuid']) && !empty($_POST['orr1_to_uuid'])) {
                $this->requestobj['orr1_to_uuid'] = $this->input->post('orr1_to_uuid');
                $this->requestobj['orr1_to_major_id'] = $this->input->post('orr1_to_major_id');
                $this->requestobj['orr1_to_minor_id'] = $this->input->post('orr1_to_minor_id');
            }
...
//                echo "<pre>";print_r($this->input->post());echo "</pre>";exit;
        $this->form_validation->set_rules('tolltype', 'Toll Type', 'trim|required');
        $this->form_validation->set_rules('toll_center_loaction', 'Toll Location', 'required');
        if ($this->input->post('tolltype') == 1) {
...
//                echo $this->form_validation->run();exit;
        if ($this->form_validation->run() == true) {
            $this->requestobj['tolltype'] = $this->input->post('tolltype');
            ...
            if ($this->input->post('tolltype') == 1)
                ...
                $this->requestobj['tc_id'] = implode(',', $this->input->post('toll_center_loaction'));
...
//            echo "<pre>";print_r($response);echo "</pre>";exit;
    ...
    public function tollcharges() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $this->requestobj['orr_charges'] = $this->input->post('orr_charges');
            $this->requestobj['tc_id'] = $this->input->post('tcn');
            $jsonObject = $this->bmt->user_params($this->requestobj);
            // echo $jsonObject;exit;
            $this->rest->header("Authtoken", $this->auth_token);
    ...
    public function updatetoll($id = NULL) {
        ...
        //exit("444");
        //~ if(isset($_POST['toll_type_id']) && $_POST['toll_type_id'] == "2"){
        //~ $this->form_validation->set_rules('orr_type1', 'Type', 'required');
        //~ $this->form_validation->set_rules('orr_type2', 'Type', 'required');
        //~ }
        //$this->form_validation->set_rules('address', 'Address', 'trim|required');
        if ($this->form_validation->run() == true) {
            ...
            $this->requestobj['lngone'] = $lng;
            $this->requestobj['latone'] = $lat;
            $this->requestobj['lngtwo'] = $_POST['lngsec'];
            $this->requestobj['lattwo'] = $_POST['latsec'];
            ...
            $this->requestobj['toll_type_id'] = $this->input->post('toll_type_id');
            if (isset($_POST['from_uuid']) && !empty($_POST['from_uuid'])) {
                $this->requestobj['from_uuid'] = $this->input->post('from_uuid');
                $this->requestobj['from_major_id'] = $this->input->post('from_major_id');
                $this->requestobj['from_minor_id'] = $this->input->post('from_minor_id');
            ...
            if (isset($_POST['orr_from_uuid']) && !empty($_POST['orr_from_uuid'])) {
                ...
                $this->requestobj['orr_from_major_id'] = $this->input->post('orr_from_major_id');
                $this->requestobj['orr_from_minor_id'] = $this->input->post('orr_from_minor_id');
                $this->requestobj['orr_from_uuid_id'] = $this->input->post('orr_from_uuid_id');
            }
            if (isset($_POST['orr1_to_uuid']) && !empty($_POST['orr1_to_uuid'])) {
                $this->requestobj['orr1_to_uuid'] = $this->input->post('orr1_to_uuid');
                $this->requestobj['orr1_to_major_id'] = $this->input->post('orr1_to_major_id');
                $this->requestobj['orr1_to_minor_id'] = $this->input->post('orr1_to_minor_id');
                $this->requestobj['orr_to_uuid_id'] = $this->input->post('orr_to_uuid_id');
            }
            $jsonObject = $this->bmt->user_params($this->requestobj);
            //echo $jsonObject; exit;
    ...
    public function get_single_semiadmin() {
        ...
        if ($this->role_id == 3) {
            $this->rest->header("Authtoken", $this->auth_token);
            $listTollCenterLocations = $this->rest->get("toll/getListTollOfCenters");
            $data['listTollCenterLocations'] = @$listTollCenterLocations->response;
        }
    ...
    public function updasemiadmindetails() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->role_id == 3) {
                ...
                if ($this->input->post('tolltype') == 2)
                    $this->requestobj['tc_id'] = $this->input->post('toll_center_loaction');
                else
...
//            $this->rest->header("Authtoken", $this->auth_token);
    ...
    public function addtolloperator() {
        $toll_staff_details = $this->rest->get("toll/listTollOperatorsWithTollCenters");
//		chackStatus($toll_staff_details);
        $this->rest->header("Authtoken", $this->auth_token);
        $listTollCenterLocations = $this->rest->get("toll/getListTollOfCenters");
//                echo "<pre>";print_r($listTollCenterLocations);echo "</pre>";exit;
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array('views' => $views, 'toll_staff_details' => @$toll_staff_details->response, 'listTollCenterLocations' => @$listTollCenterLocations->response);
            $this->load->view('toll/template/template', $data);
        }
    }
    ...
    public function lanedetails() {
        if (isset($this->session->userdata['tc_id']) && $this->session->userdata['tc_id'] != '') {
            $_POST['tc_id'] = $this->session->userdata['tc_id'];
            $this->session->unset_userdata('tc_id');
        }
        if (isset($_POST['tc_id']) && $_POST['tc_id'] != '')
            $toll_data = $this->rest->get('toll/getLoginUserTollName/' . $_POST['tc_id']);
        else
            $toll_data = $this->rest->get('toll/getLoginUserTollName');
//        echo "<pre>";print_r($toll_data);echo "</pre>";//exit;
        chackStatus($toll_data);
        $this->rest->header("Authtoken", $this->auth_token);
        $listTollCenterLocations = $this->rest->get("toll/getListTollOfCenters");
        $this->rest->header("Authtoken", $this->auth_token);
        if (isset($_POST['tc_id']) && $_POST['tc_id'] != '')
            $lane_details = $this->rest->get('toll/getLaneDetails/' . $_POST['tc_id']);
        else
            $lane_details = $this->rest->get('toll/getLaneDetails');
//        echo "<pre>";print_r($lane_details);echo "</pre>";exit;
        //$this->rest->debug();exit;
        ...
        if (!empty($lane_details->response->maped_lane_details_orr_from)) {
            foreach ($lane_details->response->maped_lane_details_orr_from as $key => $value) {
                $maped_lane_details_orr_from[$value->lane_number] = array('status_flag' => $value->status_flag);
            }
        }
        if (!empty($lane_details->response->maped_lane_details_orr_to)) {
            foreach ($lane_details->response->maped_lane_details_orr_to as $key => $value) {
                $maped_lane_details_orr_to[$value->lane_number] = array('status_flag' => $value->status_flag);
            }
        }
        //pd($to_lanes_mapped);
        //$this->rest->debug();exit;
        $views = array('toll/lane_details');
        $data = array('views' => $views, 'toll_data' => @$toll_data->response, 'lane_details' => @$lane_details->response,
            'listTollCenterLocations' => $listTollCenterLocations->response,
            'from_lanes_mapped' => @$from_lanes_mapped, 'to_lanes_mapped' => @$to_lanes_mapped,
            'orr_from_lanes_mapped' => @$maped_lane_details_orr_from, 'orr_to_lanes_mapped' => @$maped_lane_details_orr_to,
            'from_way_total_lanes' => @$from_way_total_lanes, 'to_way_total_lanes' => @$to_way_total_lanes,
            'from_way_total_lanes_bmt' => @$from_way_total_lanes_bmt, 'to_way_total_lanes_bmt' => @$to_way_total_lanes_bmt);
        if (isset($_POST['tc_id']) && $_POST['tc_id'] != '')
            $data['tc_id'] = $_POST['tc_id'];
        else
            $data['tc_id'] = reset(explode(',', $this->tc_id));
    ...
    public function updatelanedetails() {
        ...
        //~ print_r($_POST);
        //~ exit;
        $this->session->set_userdata('tc_id', $this->input->post('tc_id'));
        ...
        if (count($_POST['orrfrom']) > $_POST['orr_from_way_total_lanes_bmt']) {
            $lane_count = $_POST['orr_from_way_total_lanes_bmt'];
            $this->session->set_flashdata("errormsg", "You are Able To Check Only $lane_count Lanes From Third Entry");
            redirect('lanedetails');
            exit;
        }
        if (count($_POST['orrto']) > $_POST['orr_to_way_total_lanes_bmt']) {
            $lane_count = $_POST['orr_to_way_total_lanes_bmt'];
            $this->session->set_flashdata("errormsg", "You are Able To Check Only $lane_count Lanes From Fourth Entry");
            redirect('lanedetails');
            exit;
        }
        ...
        $this->requestobj['orr_from_lanes'] = $this->input->post('orrfrom');
        $this->requestobj['orr_to_lanes'] = $this->input->post('orrto');
...
//        echo "<pre>";print_r($response);echo "</pre>";exit;
    ...
    public function positions() {
        ...
        $csv = array();
//                $this->rest->header("Authtoken", $this->auth_token);
        $listTollCenterLocations = $this->rest->get("toll/listTollCenterLocations");

        if (isset($_POST["submit"]) && isset($_FILES)) {
            $target_dir = "uploads/";
            $target_file = $target_dir . basename($_FILES["importlocations"]["name"]);
            $fileType = pathinfo($target_file, PATHINFO_EXTENSION);
            if ($fileType != "csv") {
                $message = 1;
            } else {
                if (move_uploaded_file($_FILES["importlocations"]["tmp_name"], "tclatlongs.csv")) {
                    //chmod("/var/www/html/bookmytoll/tclatlongs.csv", 0777);
                    $handle = fopen('tclatlongs.csv', 'r');
                    set_time_limit(0);
                    $row = 0;
                    $col_count = 0;
                    ...
                    unset($csv['0']);
                    fclose($handle);
                    $data['tollcenters'] = $csv;
                    $jsonObject = $this->bmt->user_params($data);
                    $this->rest->header("Authtoken", $this->auth_token);
                    //echo $jsonObject; exit;
                    $response = $this->rest->post("toll/importData", $jsonObject);
                    $message = 2;
                }
            }
        }
        $views = array('toll/importlocations');
        $data = array('views' => $views, 'errors' => $message, 'listTollCenterLocations' => @$listTollCenterLocations->response);
        $this->load->view('toll/template/template', $data);
    }
...
//echo "<pre>"; print_r($refund_details->response);exit;
    ...
    public function getTollCenterNames() {
        $this->requestobj['toll_type_id'] = $this->input->post('tolltype_id');
        $_POST = $this->requestobj;
        $jsonObject = $this->bmt->user_params($_POST);
        //$this->rest->header("Authtoken",$this->auth_token);
        $response = $this->rest->post("toll/getTollCenterNames", $jsonObject);

        if (@$response->statuscode == 200) {
            //pd($response);
            $count_tolllocations = 0;
            $response_names = @$response->response;
            $toll_locations = "<option value=''>Toll Center Location</option>";
            foreach ($response_names as $key => $value) {
                $toll_locations .= "<option value='$value->tc_location'>$value->tc_location</option>";
                $count_tolllocations++;
            }
            if ($count_tolllocations == 0)
                $toll_locations = '<option value="">No Toll Center Names</option>';
        }
        else {
            $toll_locations = '<option value="">No Toll Center Names</option>';
        }
    ...
    public function getTollCenters() {
        $this->requestobj['toll_type_id'] = $this->input->post('tolltype_id');
        $_POST = $this->requestobj;
        $jsonObject = $this->bmt->user_params($_POST);
//        echo $jsonObject;exit;
//        $this->rest->header("Authtoken",$this->auth_token);
        $response = $this->rest->post("toll/getTollCenterNamesNew", $jsonObject);

        if (@$response->statuscode == 200) {
            //pd($response);
            $count_tolllocations = 0;
            $response_names = @$response->response;
//                        echo "<pre>";print_r($response_names);echo "</pre>";exit;
            $toll_locations = "<option value=''>Toll Center Location</option>";
            foreach ($response_names as $key => $value) {
                if ($this->requestobj['toll_type_id'] == 2)
                    $toll_locations .= "<option value='$value->tc_id'>$value->city" . " - " . "$value->tc_location" . " - " . "$value->tc_name</option>";
                else
                    $toll_locations .= "<option value='$value->tc_id'>$value->tc_location</option>";
                $count_tolllocations++;
            }
            if ($count_tolllocations == 0)
                $toll_locations = '<option value="">No Toll Center Names</option>';
      ...
      public function getAllCities() {
        ...
        $response = $this->rest->get("toll/getAllCities");
        if (@$response->statuscode == 200) {
            //pd($response);
            $count_cities = 0;
            $response_names = @$response->response;
//                        echo "<pre>";print_r($response_names);echo "</pre>";exit;
            ...
            foreach ($response_names as $key => $value) {
                $cities .= "<option value='$value->id'>$value->city</option>";
            }
        }
        else {
            $cities = '<option value="">No Cities Available</option>';
        }
        echo $cities;
        exit;
    }
