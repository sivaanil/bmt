...
//echo "<pre>";print_r($jsonObject);exit;
    ...
    public function history() {
        ...
        if ($response->statuscode == 200) {  //echo "<br/>"; print_r($response->response);exit;
...
//$this->load->view('user/basic/otp',$data);  
    ...
    public function refresh() {
        ...
        $mobileinfo = $this->rest->get('tolloperator/beacon');
        $this->rest->header("Authtoken", $this->auth_token);
        $isUpdated = $this->rest->get('tolloperator/getWays');
        $checkWhetherUpdated = $isUpdated->response;
        $data['isUpdated'] = 0;
        if ($checkWhetherUpdated->isupdated == 1) {
            $data['isUpdated'] = $checkWhetherUpdated->isupdated;
        }
        $this->rest->header("Authtoken", $this->auth_token);
        $webinfo = $this->rest->get('tolloperator/nonBeacon');
