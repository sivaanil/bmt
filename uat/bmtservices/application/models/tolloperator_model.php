...
public function getLanMapping($stafid){
   // echo $stafid;exit;
    $result_array=  array();
     $result = $this->db->select('*')->where(array('ts_id'=>$stafid))->get('bmt_lane_mapping')->result_array();
    // echo $this->db->last_query();exit;
            if(count($result)){               
            return $result_array=  array('count'=>count($result));
            }else{
            return $result_array=  array('count'=>count($result));
            }
}
...
public function getDetailsOfBeacon($stafid){
    $tcDetails = $this->db->query("select * from toll_staff where ts_id=".$stafid)->result_array();
    $tc_id = $tcDetails['0']['tc_id'];
//
...
//     +"uvd.defalt=1 and u.id=uvd.user_id and uvd.reg_type='WRU' "
            ...
            $this->db->where('toll_staff.tc_id',$tc_id);
        ...
        public function getDetailsOfBeacon1($stafid){
            $tcDetails = $this->db->query("select * from toll_staff ts join bmt_lane_mapping blm 
            on ts.ts_id = blm.ts_id
            where ts.ts_id=".$stafid)->result_array();
            $tc_id = $tcDetails['0']['tc_id'];
            $way_type = $tcDetails['0']['way_type'];
			...
			$this->db->where('transactions.tc_id',$tc_id);
			$this->db->where('transactions.way_type',$way_type);
        ...
        public function  get_ways($stafid){
            $this->db->select('toll_center.tc_id,toll_center.from_way_location,toll_center.to_way_location,toll_center.orr_from_way_location,toll_center.orr_to_way_location');
            $this->db->from('toll_staff'); 
...
public function current_date($stafid){
//$date= $data['date_wise'];
$date= date('Y-m-d');
//$mainarray=array();
