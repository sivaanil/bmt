...
class Toll extends CI_Controller {
    ...
    public function addTollCenter() {
        ...
        if ($_POST['toll_type'] == "orr") {
            $this->form_validation->set_rules('orr_type1', 'Type', 'required');
            $this->form_validation->set_rules('orr_type2', 'Type', 'required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($tc_id) {
                ...
                if ($_POST['orr_from_way_no_of_lanes'] != '') {
                    $orr_from_lanes = $_POST['from_way_no_of_lanes'];
                    for ($i = 1; $i <= $orr_from_lanes; $i++) {
                        $orr_fromlane_details[$i] = array('tc_id' => $tc_id, 'way_type' => 3, 'lane_number' => $i, 'created_date' => $_POST['tc_created_date']);
                    }
                }
                if ($_POST['orr_to_way_no_of_lanes'] != '') {
                    $orr_to_lanes = $_POST['orr_to_way_no_of_lanes'];
                    for ($i = 1; $i <= $orr_to_lanes; $i++) {
                        $orr_tolane_details[$i] = array('tc_id' => $tc_id, 'way_type' => 4, 'lane_number' => $i, 'created_date' => $_POST['tc_created_date']);
                    }
                }
                $this->lanes->createLanes($fromlane_details);

                if (!empty($tolane_details)) {
                    $this->lanes->createLanes($tolane_details);
                }
                if (!empty($orr_fromlane_details)) {
                    $this->lanes->createLanes($orr_fromlane_details);
                }
                if (!empty($orr_tolane_details)) {
                    $this->lanes->createLanes($orr_tolane_details);
                }
    ...
    public function updateTollCenter() {
        ...
        //$this->form_validation->set_rules('to_way_no_of_bmt_lanes', 'Number Of Lanes From Bmt', 'trim|required|xss_clean|max_length[9]|callback_checkNumberOfToLanes');
        $this->form_validation->set_rules('assigned_id', 'Number Of Lanes From Bmt', 'trim|required|xss_clean|numeric');
        //~ if ($_POST['toll_type_id'] == "2") {
            //~ $this->form_validation->set_rules('orr_type1', 'Type', 'required');
            //~ $this->form_validation->set_rules('orr_type2', 'Type', 'required');
        //~ }
        //~ if (isset($_POST['from_uuid'])) {
            //~ $this->form_validation->set_rules('from_uuid', 'From Becon UUID', 'trim|required|max_length[250]');
            //~ $this->form_validation->set_rules('from_major_id', 'From Becon Major Id', 'trim|required|max_length[10]|callback_updatecheckFromBeconMajorId|callback_checkBeconMajorId');
            //~ $this->form_validation->set_rules('from_minor_id', 'From Becon Minor Id', 'trim|required|max_length[10]|callback_updatecheckFromBeconMinarId|callback_checkBeconMinarId');
            //~ //$this->form_validation->set_rules('from_uuid_id', 'From Becon UUID Id', 'trim|required');
        //~ }
        //~ if (isset($_POST['to_uuid'])) {
            //~ $this->form_validation->set_rules('to_uuid', 'To Becon UUID', 'trim|required|max_length[250]');
            //~ $this->form_validation->set_rules('to_major_id', 'To Becon Major Id', 'trim|required|max_length[10]|callback_updatecheckToBeconMajorId|callback_checkBeconMajorId');
            //~ $this->form_validation->set_rules('to_minor_id', 'To Becon Minor Id', 'trim|required|max_length[10]|callback_updatecheckToBeconMinarId|callback_checkBeconMinarId');
            //~ //$this->form_validation->set_rules('to_uuid_id', 'To Becon UUID Id', 'trim|required');
        //~ }
        //~ if (isset($_POST['orr_from_uuid'])) {
		//~ $this->form_validation->set_rules('orr_from_major_id', 'Orr From Becon Major Id', 'trim|required|max_length[10]|callback_checkToBeconMajorId|callback_checkBeconMajorId');
        //~ $this->form_validation->set_rules('orr_from_minor_id', 'Orr From Becon Minor Id', 'trim|required|max_length[10]|callback_checkToBeconMinarId|callback_checkBeconMinarId');
        //~ }
        //~ if (isset($_POST['orr1_to_uuid'])) {
		//~ $this->form_validation->set_rules('orr1_to_major_id', 'Orr To Becon Major Id', 'trim|required|max_length[10]|callback_checkToBeconMajorId|callback_checkBeconMajorId');
        //~ $this->form_validation->set_rules('orr1_to_minor_id', 'Orr To Becon Minor Id', 'trim|required|max_length[10]|callback_checkToBeconMinarId|callback_checkBeconMinarId');
        //~ }
        if ($this->form_validation->run() == false) {
            ...
            $where_condition = array('tc_id' => $_POST['tc_id']);
            if ($_POST['toll_type_id'] == '2') {
                $data = array('tc_name' => $_POST['tc_name'], 'tc_location' => $_POST['tc_location'], 'from_way_no_of_lanes' => $_POST['from_way_no_of_lanes'], 'from_way_no_of_bmt_lanes' => $_POST['from_way_no_of_bmt_lanes'], 'from_way_location' => $_POST['from_way_location'], 'to_way_location' => $_POST['to_way_location'], 'to_way_no_of_lanes' => $_POST['to_way_no_of_lanes'], 'to_way_no_of_bmt_lanes' => $_POST['to_way_no_of_bmt_lanes'], 'lng' => $_POST['lngone'], 'lat' => $_POST['latone'], 'address' => $_POST['address'], 'to_lat' => $_POST['lattwo'], 'to_lag' => $_POST['lngtwo'],
                    'orr_from_way_location' => $_POST['orr_from_way_location'], 'orr_to_way_location' => $_POST['orr_to_way_location'], 'orr_from_way_no_of_lanes' => $_POST['orr_from_way_no_of_lanes'],
                    'orr_from_way_no_of_bmt_lanes' => $_POST['orr_from_way_no_of_bmt_lanes'], 'orr_to_way_no_of_lanes' => $_POST['orr_to_way_no_of_lanes'], 'orr_to_way_no_of_bmt_lanes' => $_POST['orr_to_way_no_of_bmt_lanes']);
            } else {
                $data = array('tc_name' => $_POST['tc_name'], 'tc_location' => $_POST['tc_location'], 'from_way_no_of_lanes' => $_POST['from_way_no_of_lanes'], 'from_way_no_of_bmt_lanes' => $_POST['from_way_no_of_bmt_lanes'], 'from_way_location' => $_POST['from_way_location'], 'to_way_location' => $_POST['to_way_location'], 'to_way_no_of_lanes' => $_POST['to_way_no_of_lanes'], 'to_way_no_of_bmt_lanes' => $_POST['to_way_no_of_bmt_lanes'], 'lng' => $_POST['lngone'], 'lat' => $_POST['latone'], 'address' => $_POST['address'], 'to_lat' => $_POST['lattwo'], 'to_lag' => $_POST['lngtwo']);
            }
            ...
            if (updaterecord($table_name, $where_condition, $data)) {
                ...
                $table_name = 'latlng';
                $where_condition = array('tc_id' => $_POST['tc_id'], 'way' => 1);
                $data = array('lng' => $_POST['lngone'], 'lat' => $_POST['latone']);
                updaterecord($table_name, $where_condition, $data);

                $table_name = 'latlng';
                $where_condition = array('tc_id' => $_POST['tc_id'], 'way' => 2);
                $data = array('lng' => $_POST['lngtwo'], 'lat' => $_POST['lattwo']);
                updaterecord($table_name, $where_condition, $data);

                if (isset($_POST['from_uuid'])) {
                    if ($_POST['from_uuid_id'] != '') {
                        $becon_one = array('uuid' => $_POST['from_uuid'], 'major_id' => $_POST['from_major_id'], 'minor_id' => $_POST['from_minor_id'], 'entry_type' => strtoupper($_POST['nh_type1']));
                        $table_name = 'beacons';
                        $where_condition = array('beacon_id' => $_POST['from_uuid_id']);
                        updaterecord($table_name, $where_condition, $becon_one);
                    } else {

                        $becon_one = array('tc_id' => $_POST['tc_id'], 'uuid' => $_POST['from_uuid'], 'major_id' => $_POST['from_major_id'], 'minor_id' => $_POST['from_minor_id'], 'entry_type' => strtoupper($_POST['nh_type1']));
                        $this->db->insert('beacons', $becon_one);
                        $insert_id = $this->db->insert_id();
                        $data = array('from_way_beacon_id' => $insert_id);
                        $table_name = 'toll_center';
                        $where_condition = array('tc_id' => $_POST['tc_id']);
                        updaterecord($table_name, $where_condition, $data);
                    }
                ...
                if (isset($_POST['to_uuid'])) {
                    if ($_POST['to_uuid_id'] != '') {
                        $becon_two = array('uuid' => $_POST['to_uuid'], 'major_id' => $_POST['to_major_id'], 'minor_id' => $_POST['to_minor_id'], 'entry_type' => strtoupper($_POST['nh_type2']));
                        $table_name = 'beacons';
                        $where_condition = array('beacon_id' => $_POST['to_uuid_id']);
                        updaterecord($table_name, $where_condition, $becon_two);
                    } else {
                        //exit("dsdssd");
                        $becon_two = array('tc_id' => $_POST['tc_id'], 'uuid' => $_POST['to_uuid'], 'major_id' => $_POST['to_major_id'], 'minor_id' => $_POST['to_minor_id'], 'entry_type' => strtoupper($_POST['nh_type2']));
                        $this->db->insert('beacons', $becon_two);
                        $insert_id = $this->db->insert_id();
                        $data = array('to_way_beacon_id' => $insert_id);
                        $table_name = 'toll_center';
                        $where_condition = array('tc_id' => $_POST['tc_id']);
                        updaterecord($table_name, $where_condition, $data);
                    }
                ...
                if (isset($_POST['orr_from_uuid'])) {
                    if ($_POST['orr_from_uuid_id'] != '') {
                        $becon_two = array('uuid' => $_POST['orr_from_uuid'], 'major_id' => $_POST['orr_from_major_id'], 'minor_id' => $_POST['orr_from_minor_id'], 'entry_type' => strtoupper($_POST['orr_type1']));
                        $table_name = 'beacons';
                        $where_condition = array('beacon_id' => $_POST['orr_from_uuid_id']);
                        updaterecord($table_name, $where_condition, $becon_two);
                    } else {
                        $becon_two = array('tc_id' => $_POST['tc_id'], 'uuid' => $_POST['orr_from_uuid'], 'major_id' => $_POST['orr_from_major_id'], 'minor_id' => $_POST['orr_from_minor_id'], 'entry_type' => strtoupper($_POST['orr_type1']));
                        $this->db->insert('beacons', $becon_two);
                        $insert_id = $this->db->insert_id();
                        $data = array('orr_from_way_beacon_id' => $insert_id);
                        $table_name = 'toll_center';
                        $where_condition = array('tc_id' => $_POST['tc_id']);
                        updaterecord($table_name, $where_condition, $data);
                    }
                ...
                if (isset($_POST['orr1_to_uuid'])) {
                    if ($_POST['orr_to_uuid_id'] != '') {
						$becon_two = array('uuid' => $_POST['orr1_to_uuid'], 'major_id' => $_POST['orr1_to_major_id'], 'minor_id' => $_POST['orr1_to_minor_id'], 'entry_type' => strtoupper($_POST['orr_type2']));
                        $table_name = 'beacons';
                        $where_condition = array('beacon_id' => $_POST['orr_to_uuid_id']);
                        updaterecord($table_name, $where_condition, $becon_two);
                    } else {
						$becon_two = array('tc_id' => $_POST['tc_id'], 'uuid' => $_POST['orr1_to_uuid'], 'major_id' => $_POST['orr1_to_major_id'], 'minor_id' => $_POST['orr1_to_minor_id'], 'entry_type' => strtoupper($_POST['orr_type2']));
                        $this->db->insert('beacons', $becon_two);
                        $insert_id = $this->db->insert_id();
                        $data = array('orr_to_way_beacon_id' => $insert_id);
                        $table_name = 'toll_center';
                        $where_condition = array('tc_id' => $_POST['tc_id']);
                        updaterecord($table_name, $where_condition, $data);
                    }
                ...
                if ($_POST['orr_from_way_no_of_lanes'] != '') {
                    $orr_from_lanes = $_POST['orr_from_way_no_of_lanes'];
                    for ($i = 1; $i <= $orr_from_lanes; $i++) {
                        $orr_fromlane_details[$i] = array('tc_id' => $tc_id, 'way_type' => 1, 'lane_number' => $i, 'created_date' => $_POST['tc_created_date']);
                    }
                }
                if ($_POST['orr_to_way_no_of_lanes'] != '') {
                    $orr_to_lanes = $_POST['orr_to_way_no_of_lanes'];
                    for ($i = 1; $i <= $orr_to_lanes; $i++) {
                        $orr_tolane_details[$i] = array('tc_id' => $tc_id, 'way_type' => 2, 'lane_number' => $i, 'created_date' => $_POST['tc_created_date']);
                    }
                }
    ...
    public function getListTollOfCenters() {
        $data = $this->toll->getListTollOfCenters($this->login_user_id);
        $result['TollCenterDetails'] = $data;
        $this->rest->response(responseObject(SUCCESS_CODE, '', $result, success, ''));
        return;
    }
    ...
    public function getSpecificTollDetails($toll_id) {
        ...
        //$data = $this->db->query('select * from toll_center tc join city c on tc.city_id = c.id where tc.tc_id = ' . ($toll_id))->result_array();
    ...
    public function addSemiAdmin() {
        $_POST = $this->requestObject;
        $tolltype = $_POST['tolltype'];
        unset($_POST['tolltype']);
...
//        }
        if ($this->login_role_id != 3)
            $this->form_validation->set_rules('tc_id', 'Toll Center Name', 'required|callback_checkTollLocationExisting');
            ...
            if ($last_inserted_id) {
                if ($this->login_role_id == 1) {
                    ...
                    foreach ($tc_ids as $key => $value) {
                        ...
                        $table_name = 'toll_staff';
                        $where_condition = array('tc_id' => $value, 'roll_id' => 4);
    ...
    public function checkTollLocationExisting($location_id) {
        ...
        foreach ($location_ids as $key => $value) {
            $data = $this->toll->checksemi($value);
            if (!$data) {
                $tcData = $this->toll->getTollDataById($value);
                $tollNames .= $tcData[0]['tc_name'] . ' - ' . $tcData[0]['tc_location'] . ", ";
                $return = false;
            }
        }
        if (!$return) {
            $tollNames = trim($tollNames, ', ');
            $this->form_validation->set_message('checkTollLocationExisting', "The Toll Location(s) " . $tollNames . " Already Assigned");
        }
        return $return;
    }
...
//        }
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (updaterecord($table_name, $where_condition, $data)) {
                if ($this->login_role_id == 1) {
                    ...
                    $tc_ids = explode(',', $user_previous_data['tc_id']);
                    foreach ($tc_ids as $key => $value) {
                        /* update Toll center as active */
                        $table_name = 'toll_center';
                        $where_condition = array('tc_id' => $value);
                        ...
                        $table_name = 'toll_staff';
                        $where_condition = array('tc_id' => $value, 'roll_id' => 4);
                        $data = array('status_flag' => 1, 'assigned_id' => 1);
                        updaterecord($table_name, $where_condition, $data);
                    }
                    ...
                    foreach ($tc_ids as $key => $value) {
                        ...
                        $table_name = 'toll_staff';
                        $where_condition = array('tc_id' => $value, 'roll_id' => 4);
                        $data = array('status_flag' => 0, 'assigned_id' => $ts_id);
                        updaterecord($table_name, $where_condition, $data);
                    }
                }
    ...
    public function updatecheckTollLocationExisting($location_id) {
        ...
        $tollNames = '';
        foreach ($location_ids as $key => $value) {
            $data = $this->toll->checksemi($value, $_POST['ts_id']);
            if (!$data) {
                $tcData = $this->toll->getTollDataById($value);
                $tollNames .= $tcData[0]['tc_name'] . ' - ' . $tcData[0]['tc_location'] . ", ";
                $return = false;
        ...
        if (!$return) {
            $tollNames = trim($tollNames, ', ');
            $this->form_validation->set_message('updatecheckTollLocationExisting', "The Toll Location(s) " . $tollNames . " Already Assigned");
        }
        return $return;
    }
    ...
    public function listSemiAdmins() {
        $data = $this->tollstaff->listSemiAdminsNew($this->login_user_id);
        $this->rest->response(responseObject(SUCCESS_CODE, '', $data, success, ''));
        return;
    }
    ...
    public function listTollOperatorsWithTollCenters() {
        $data = $this->tollstaff->listTollOperatorsWithTollCenters($this->login_user_id);
        $this->rest->response(responseObject(SUCCESS_CODE, '', $data, success, ''));
        return;
    }
    ...
    public function activrOrInactiveAdmin($user_id, $status) {
        ...
        if ($status == 0) {
            $tc_id_of_user_id_status = $this->toll->getTcIdByUseridNew($user_id, $tc_id);
            if ($tc_id_of_user_id_status['status'] == 0) {
                $this->rest->response(responseObject(409, 'The Toll(s) ' . $tc_id_of_user_id_status['TollCenters'] . ' Assigned To Some Other Else', '', fail, ''));
                return;
            }
    ...
    public function getLoginUserTollName($tc_id = '') {
        $data = $this->toll->getLoginUserTollName($this->login_user_id, $tc_id);
        $this->rest->response(responseObject(SUCCESS_CODE, '', $data, success, ''));
        return;
    }

    public function getLaneDetails($tc_id = '') {
        $data = $this->toll->getLaneDetails($this->login_user_id, $tc_id);
        $data['maped_lane_details_from'] = $this->lanes->getMappedLanesFrom($this->login_user_id, $tc_id);
        $data['maped_lane_details_to'] = $this->lanes->getMappedLanesTo($this->login_user_id, $tc_id);
        $data['maped_lane_details_orr_from'] = $this->lanes->getMappedLanesOrrFrom($this->login_user_id, $tc_id);
        $data['maped_lane_details_orr_to'] = $this->lanes->getMappedLanesOrrTo($this->login_user_id, $tc_id);
        $this->rest->response(responseObject(SUCCESS_CODE, '', $data, success, ''));
        return;
    }
...
//        }
        ...
        $where_condition = array('tc_id' => $_POST['tc_id']);
        // Delete old mapped records
        $ts = "delete from bmt_lanes where `tc_id`=" . $_POST['tc_id'];
        $tq = $this->db->query($ts);
        //insert new records
        $data = array('status_flag' => 1);
        updaterecord($table_name, $where_condition, $data);
        if (isset($_POST['from_lanes']) && !empty($_POST['from_lanes'])) {
            foreach ($_POST['from_lanes'] as $key => $value) {
                $insertarr = array('tc_id' => $_POST['tc_id'], 'lane_number' => $value, 'way_type' => 1, 'status_flag' => 0);
                $this->db->insert('bmt_lanes', $insertarr);
            }
        }
        if (isset($_POST['to_lanes']) && !empty($_POST['to_lanes'])) {
            foreach ($_POST['to_lanes'] as $key => $value) {
                $insertarr = array('tc_id' => $_POST['tc_id'], 'lane_number' => $value, 'way_type' => 2, 'status_flag' => 0);
                $this->db->insert('bmt_lanes', $insertarr);
            }
        }
        if (isset($_POST['orr_from_lanes']) && !empty($_POST['orr_from_lanes'])) {
            foreach ($_POST['orr_from_lanes'] as $key => $value) {
                $insertarr = array('tc_id' => $_POST['tc_id'], 'lane_number' => $value, 'way_type' => 3, 'status_flag' => 0);
                $this->db->insert('bmt_lanes', $insertarr);
            }
        }
        if (isset($_POST['orr_to_lanes']) && !empty($_POST['orr_to_lanes'])) {

            foreach ($_POST['orr_to_lanes'] as $key => $value) {
                $insertarr = array('tc_id' => $_POST['tc_id'], 'lane_number' => $value, 'way_type' => 4, 'status_flag' => 0);
                $this->db->insert('bmt_lanes', $insertarr);
            }
        }
    ...
    public function refundStatus() {
        $data = $this->toll->refund_users();
        if (count($data > 0)) {
            $this->rest->response(responseObject(SUCCESS_CODE, '', $data, success, 'Refund Trasactions'));
            return;
        } else {
            $this->rest->response(responseObject(SERVERERROR_CODE, SERVER_ERROR, '', fail, ''));
            return;
    ...
    public function superConsolldatedByCurrentDate() {
        $data = $this->toll->current_date($this->login_user_id);
        // pd($data);exit;
        if (count($data) > 0) {
            // pd($data);exit;
...
//echo "dfsf";exit;
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = $this->toll->day_wise($_POST, $this->login_user_id);
            // pd($data);exit;
            if (count($data) > 0) {
                // pd($data);exit;
                $this->rest->response(responseObject(SUCCESS_CODE, '', $data, success, 'Date Wise History'), SUCCESS_CODE);
    ...
    public function consolldatedByPeriod() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = $this->toll->period_wise($_POST, $this->login_user_id);
            //pd($data);exit;
            if (count($data) > 0) {
                //pd($data);exit;
    ...
    public function getTollCenterNames() {
        $this->requestObject = $this->rest->post("request");
        $_POST = $this->requestObject['toll_type_id'];
        $data = $this->toll->gettollcenternames($_POST);
        if (count($data) > 0) {
            //pd($data);exit;
    ...
    public function getTollCenterNamesNew() {
        $this->requestObject = $this->rest->post("request");
        $_POST = $this->requestObject['toll_type_id'];
        $data = $this->toll->gettollcenternamesNew($_POST);
        if (count($data) > 0) {
            //pd($data);exit;
            $this->rest->response(responseObject(SUCCESS_CODE, '', $data, success, ''), SUCCESS_CODE);
            return;
    ...
    public function importData() {
        ...
        $fData = $data['tollcenters'];
        $this->db->query('truncate table tollcenter_coordinates');
        if (count($fData > 0)) {
            for ($i = 1; $i <= count($fData); $i++) {
                //echo $fData[$i]['1']; exit;
                $insertarr = array('tc_id' => $fData[$i]['0'], 'coordinates' => $fData[$i]['1'], 'radius' => $fData[$i]['2'], 'beacon_id' => $fData[$i]['3']);
                $this->db->insert('tollcenter_coordinates', $insertarr);
            }
            $this->rest->response(responseObject(SUCCESS_CODE, '', $data, success, ''), SUCCESS_CODE);
        } else {
            $this->rest->response(responseObject(UNAUTHORIZED_CODE, '', 'No data found in uploaded file', fail, UNAUTHORIZED), UNAUTHORIZED_CODE);
        }
    }

    public function getAllCities() {
        $data = $this->toll->getallcities();
        if (count($data) > 0) {
            //pd($data);exit;
            $this->rest->response(responseObject(SUCCESS_CODE, '', $data, success, ''), SUCCESS_CODE);
            return;
