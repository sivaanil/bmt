...
class REST 
{
    ...
    */
    ...
    public function __construct($config)
    {
        ...
        if ($this->CI->config->item('rest_ip_blacklist_enabled')) {
    ...
    // Skip keys test for $config['auth_override_class_method']['class'['method'] = 'none'
        ...
        if (!$this->CI->input->is_ajax_request() and config_item('rest_ajax_only')) {
    ...
    public function response($data = null, $http_code = null, $continue = false)
    {
        ...
        // Otherwise (if no data but 200 provided) or some data, carry on camping!
            ...
            if (method_exists($this, '_format_'.$this->response->format)) {
                ...
                header('Content-Type: '.$this->_supported_formats[$this->response->format] . '; charset=' . strtolower($this->CI->config->item('charset')));
                $output = $this->{'_format_'.$this->response->format}($data);
            }

            // If the format method exists, call and return the output in that format
            elseif (method_exists($this->CI->format, "to_".$this->response->format)) {
        ...
        if ( ! $this->_zlib_oc && ! $this->CI->config->item('compress_output')) {
    ...
    {
        if ($this->CI->input->server('CONTENT_TYPE')) {
            // Check all formats against the HTTP_ACCEPT header
            foreach ($this->_supported_formats as $format => $mime) {
                if (strpos($match = $this->CI->input->server('CONTENT_TYPE'), ';')) {
    ...
    protected function _detect_output_format()
    {
        ...
        $matches = array();
        if (!$this->_get_args and preg_match($pattern, $this->CI->uri->uri_string(), $matches)) {
            return $matches[1];
        }
        ...
        if ($this->CI->config->item('rest_ignore_http_accept') === false and $this->CI->input->server('HTTP_ACCEPT')) {
            // Check all formats against the HTTP_ACCEPT header
            foreach (array_keys($this->_supported_formats) as $format) {
                // Has this format been requested?
                if (strpos($this->CI->input->server('HTTP_ACCEPT'), $format) !== false) {
                    ...
                    // HTML or XML have shown up as a match
                        ...
                        if ($format == 'html' and strpos($this->CI->input->server('HTTP_ACCEPT'), 'xml') === false) {
                            return $format;
                        }

                        // If it is truly XML, it wont want any HTML
                        elseif ($format == 'xml' and strpos($this->CI->input->server('HTTP_ACCEPT'), 'html') === false) {
                            return $format;
                        }
                    }
                }
    ...
    {
        if ( ! $lang = $this->CI->input->server('HTTP_ACCEPT_LANGUAGE')) {
            return null;
        }
    ...
    protected function _check_limit($controller_method)
    {
        ...
        $result = $this->rest->db
                ->where('uri', $this->CI->uri->uri_string())
    ...
    protected function _auth_override_check()
    {
        ...
        $this->overrides_array = $this->CI->config->item('auth_override_class_method');
    ...
    {
        if (CI_VERSION < 2) {
            return $process ? $this->CI->input->xss_clean($val) : $val;
        }

        return $process ? $this->CI->security->xss_clean($val) : $val;
    }
    ...
    protected function _perform_ldap_auth($username = '', $password = null)
    {
        ...
        $ldaptimeout = $this->CI->config->item('timeout', 'ldap');
        $ldaphost = $this->CI->config->item('server', 'ldap');
        $ldapport = $this->CI->config->item('port', 'ldap');
        $ldaprdn = $this->CI->config->item('binduser', 'ldap');
        $ldappass = $this->CI->config->item('bindpw', 'ldap');
        $ldapbasedn = $this->CI->config->item('basedn', 'ldap');

        log_message('debug', 'LDAP Auth: Connect to ' . $ldaphost);

        $ldapconfig['authrealm'] = $this->CI->config->item('domain', 'ldap');
    ...
    protected function _check_login($username = '', $password = false)
    {
        ...
        if (!$this->CI->config->item('auth_source') && $rest_auth == 'digest') { // for digest we do not have a password passed as argument
    ...
    {
        if (strtolower( $this->CI->config->item('rest_auth') ) == 'basic') {
            header('WWW-Authenticate: Basic realm="'.$this->CI->config->item('rest_realm').'"');
        } elseif (strtolower( $this->CI->config->item('rest_auth') ) == 'digest') {
            header('WWW-Authenticate: Digest realm="'.$this->CI->config->item('rest_realm').'", qop="auth", nonce="'.$nonce.'", opaque="'.md5($this->CI->config->item('rest_realm')).'"');
        }
    ...
    public function validateRequestObject($definedArray, $requestArray){
        
        $testvar = array_diff($definedArray, array_keys($requestArray));
        if(count($testvar) || ($requestArray == false) ){

           $this->response(responseObject(BADREQUEST_CODE,implode("---",$testvar),'','fail',BAD_REQUEST));
           return; 
        } 
    }
}