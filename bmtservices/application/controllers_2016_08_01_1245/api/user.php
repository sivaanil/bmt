...
//                echo $query[0]['toll_type_id']; exit;
                ...
                elseif ($query[0]['toll_type_id'] == 2) {
                    ...
                    if (count($tq3) > 0) {
                        ...
                        if ($oldExitedId == $tcid) {
                            $ts2 = "select transaction_id from transactions where `transaction_date` >= date_sub(now(), interval 2 minute) AND `transaction_date` < now() AND tc_id=" . $tcid . " AND user_id=" . $this->login_user_id . " AND vehicle_id=" . $vid . " AND paid_status=1";
					...
					VALUES ('" . json_encode($data) . "')";
                        ...
                        if ($ringroad['0']->charge != '') {
                            ...
                            //echo '<pre>';
                            //print_r($query); exit;
                            $lanedetails = $this->db->query("select * from bmt_lanes where user_selsect_status = 1 AND tc_id=" . $query['0']['tc_id'])->result_array();
                            $laneno = $lanedetails['0']['lane_number'];
                            $data['html_content'] = "http://bookmytoll.com/payment?total_amount=" . $amount . "&transaction_id=" . $order_id . "&user_id=" . $this->login_user_id . "&Authtoken=" . $authToken . "&lane_number=" . $laneno;
                            $data['detected_by'] = "Beacon";
                            $userVehicleNo = $this->db->query("select * from vehicles where vehicle_id=" . $_POST['vehicle_id'])->result_array();
                ...
                $CI->db->where('tc_id', $_POST['toll_id']);
                $query = $CI->db->get()->result_array();
                ...
                if ($query[0]['toll_type_id'] == 1) {
					...
					VALUES ('User already detected')";
                        ...
                        if (!$ringroad) {
                            /* -------------- Helpful for debugging purpose-------------------- */
                            $sql1 = "INSERT INTO `response_log`(`response`) 
					VALUES ('Please exit from Ringroad')";
                            $result1 = $this->db->query($sql1);
                            /* -------------- Helpful for debugging purpose ends-------------------- */
                            $this->rest->response(responseObject(CONFLICT_CODE, '', '', nocontent, 'Please exit from Ringroad'), CONFLICT_CODE);
                            return;
