...
//exit;
    ...
    public function getLaneDetails() {
        ...
        $data['maped_lane_details_orr_from'] = $this->lanes->getMappedLanesOrrFrom($this->login_user_id);
        $data['maped_lane_details_orr_to'] = $this->lanes->getMappedLanesOrrTo($this->login_user_id);
        $this->rest->response(responseObject(SUCCESS_CODE, '', $data, success, ''));
        return;
    }
    ...
    public function importData() {
        ...
        if (count($fData > 0)) {
            for ($i = 1; $i <= count($fData); $i++) {
                //echo $fData[$i]['1']; exit;
                $insertarr = array('tc_id' => $fData[$i]['0'], 'coordinates' => $fData[$i]['1'], 'radius' => $fData[$i]['2']);
