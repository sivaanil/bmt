...
class Bmtlanes_model extends CI_Model {
    ...
    public function getMappedLanesFrom($login_user_id, $tc_id = '') {
        $result = array();
        if ($tc_id != '')
            $query = "SELECT bmt_lanes.* FROM bmt_lanes WHERE bmt_lanes.tc_id= $tc_id AND bmt_lanes.way_type = 1";
        else
    ...
    public function getMappedLanesTo($login_user_id, $tc_id = '') {
        $result = array();
        if ($tc_id != '')
            $query = "SELECT bmt_lanes.* FROM bmt_lanes WHERE bmt_lanes.tc_id= $tc_id AND bmt_lanes.way_type = 2";
        else
    ...
    public function getMappedLanesOrrFrom($login_user_id, $tc_id = '') {
        $result = array();
        if ($tc_id != '')
            $query = "SELECT bmt_lanes.* FROM bmt_lanes WHERE bmt_lanes.tc_id= $tc_id AND bmt_lanes.way_type = 3";
        else
    ...
    public function getMappedLanesOrrTo($login_user_id, $tc_id = '') {
        $result = array();
        if ($tc_id != '')
            $query = "SELECT bmt_lanes.* FROM bmt_lanes WHERE bmt_lanes.tc_id= $tc_id AND bmt_lanes.way_type = 4";
        else
