...
//        $this->login_user_id      =  $this->login_user_details['user_id'];
    ...
    public function getWays() {
        ...
        $data['isupdated'] = 0;
        $tollDetails = $this->db->query("select tc_id from toll_staff where ts_id = " . $this->login_user_id)->result_array();
        //~ echo '<pre>';
        //~ print_r($tollDetails);
        //~ exit;
        $tcid = $tollDetails['0']['tc_id'];
        $tollCenterDetails = $this->db->query("select toll_type_id from toll_center where tc_id = " . $tcid)->result_array();
        $data['tollTypeId'] = $tollCenterDetails['0']['toll_type_id'];
        $isTollUpdated = $this->db->query("select * from bmt_lanes where status_flag=1 && tc_id = " . $tcid)->result_array();
        $laneData = $this->db->query("select * from bmt_lane_mapping as blm where blm.ts_id = " . $this->login_user_id)->result_array();
        if(count($laneData)>0)
            $isLaneUpdated = $this->db->query("select * from bmt_lanes as bl,bmt_lane_mapping as blm where blm.lane_id = bl.lane_id and blm.ts_id = " . $this->login_user_id)->result_array();
        if (count($isTollUpdated) > 0 || (count($laneData)>0 && count($isLaneUpdated) == 0)) {
            $data['isupdated'] = 1;
            $this->db->query("delete from bmt_lane_mapping where ts_id=" . $this->login_user_id . " && tc_id = " . $tcid);
    ...
    public function paidTransaction() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($returnid > 0) {    //pd($returnid);exit;
                if ($returnid == 4) {
