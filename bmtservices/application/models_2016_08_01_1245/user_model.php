        ...
        if($count>0)
        {
            ...
            if($vehiclenum>0)
            {
                ...
                else
                {
                    ...
                    // Main testing do here
                    $ts="select transaction_id from transactions where `transaction_date` >= date_sub(now(), interval 2 minute) AND `transaction_date` < now() AND tc_id=".$tcid." AND user_id=".$uid." AND vehicle_id=".$vid." AND way_type=".$way_type." AND paid_status=1";
                    $tq=$this->db->query($ts);
                    $tc = $tq->num_rows();
                    if($tc)
                    {
                        $msg="You have already done transaction in this way. Try after 2 Minutes.";
                        pushNote($deviceid,$msg);
                    }
	...
	//   	 TO insert exit type
				...
				if(empty($insertStatus)){
					...
					if($enterdTollGateId){
						...
						select * from orr_toll_center otc
					...
					where v.vehicle_id ="'.$vehicle_id.'" AND 
